...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  private recalcularTodo() {
    ...
    if (this.totalEnergia > 5000) {
      this.insights.push('⚠️ Consumo energético elevado en total');
    }

    if (this.totalAgua > 20000) {
      this.insights.push('💧 Alto consumo de agua detectado');
    }
  }
  ...
  calcularKPIs() {
    ...
    this.barEnergia = Math.min(
      100,
      Math.max(0, Math.round((this.totalEnergia / refE) * 100))
    );

    this.barAgua = Math.min(
      100,
      Math.max(0, Math.round((this.totalAgua / refA) * 100))
    );

    this.barCo2 = Math.min(
      100,
      Math.max(0, Math.round((this.totalCo2 / refC) * 100))
    );

    this.barResiduos = Math.min(
      100,
      Math.max(0, Math.round((this.totalResiduos / refR) * 100))
    );
  }
  ...
  calcularTrends() {
    ...
    this.trendEnergia = this.trend(
      last.energia,
      prev.energia,
      true
    );

    this.trendAgua = this.trend(
      last.agua,
      prev.agua,
      true
    );

    this.trendCo2 = this.trend(
      last.co2,
      prev.co2,
      true
    );

    this.trendResiduos = this.trend(
      last.residuos,
      prev.residuos,
      true
    );
  ...
  calcularEstadisticas() {
    ...
    if (n >= 2) {
      ...
      this.reduccionEnergia = Math.min(
        100,
        Math.max(
          -100,
          Math.round(
            (
              (this.impactos.at(-1)!.energia - e0) / e0
            ) * 100
          )
        )
      );
      ...
      this.reduccionCo2 = Math.min(
        100,
        Math.max(
          -100,
          Math.round(
            (
              (this.impactos.at(-1)!.co2 - c0) / c0
            ) * 100
          )
        )
      );
  ...
  calcularIndice() {
    ...
    this.indicePorcentaje = Math.min(
      100,
      Math.max(0, Math.round(pts / 4))
    );
  ...
  calcularGamificacion() {
    ...
    this.puntos = this.gamificacionService.calcularPuntos(
      this.impactos
    );

    this.medallas = this.gamificacionService.obtenerMedallas(
      this.puntos,
      this.impactos
    );

    this.nivel = this.gamificacionService.calcularNivel(
      this.puntos
    );

    this.rankingService.guardarPuntuacion(
      this.nombreEmpresa,
      this.puntos
    );
    ...
    if (this.nivel === 'Oro') {
      ...
      this.progresoNivel = Math.min(
        100,
        Math.max(
          0,
          Math.round(
            ((this.puntos - 300) / 300) * 100
          )
        )
      );
      ...
      this.progresoNivel = Math.min(
        ...
        Math.max(
          0,
          Math.round((this.puntos / 300) * 100)
        )
      );
    }
  }
  ...
  crearGraficoLineas() {
    ...
    this.graficoLineas = new Chart(
      this.graficoLineasRef.nativeElement,
      {
        type: 'line',

        data: {

          labels: this.impactos.map(
            (_, i) => `Reg. ${i + 1}`
          ),
  ...
  crearGraficoDona() {
    ...
    this.graficoDona = new Chart(
      this.graficoDonaRef.nativeElement,
      {
        type: 'doughnut',
