...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  private storageKey = 'impactos_anonimo';
  private idEmpresa: number | null = null;

  impactos: any[] = [];

  sinDatos = false;
  // CORREGIDO: cargando empieza en true siempre para mostrar spinner desde el inicio
  cargando = true;
  ...
  ngOnInit() {
    ...
    // CORREGIDO: usar AuthService para obtener usuario y la clave de sesión correcta
    ...
    // La clave siempre es única por empresa (via AuthService)
    this.storageKey = this.auth.getStorageKey();

    // CARGAR CACHE — si hay datos en caché los mostramos inmediatamente
    // pero igualmente consultamos el backend para actualizar
    ...
    if (cached) {
      try {
        ...
        if (Array.isArray(cachedData) && cachedData.length > 0) {
          this.impactos = cachedData;
          this.sinDatos = false;
          this.recalcularTodo();
          // Los gráficos se dibujarán tras ngAfterViewInit o tras la llamada al backend
        }
      } catch (_) { /* caché corrupta, ignorar */ }
    }

    // CARGAR BACKEND — siempre, para tener datos frescos
    if (this.idEmpresa) {
      ...
      this.empresaService.getMediciones(this.idEmpresa).subscribe({
        ...
        next: (data) => {
          ...
          // CORREGIDO: siempre apagar el spinner aquí
          this.cargando = false;
        ...
        error: () => {
          // CORREGIDO: apagar spinner también en error
          this.cargando = false;

          // Si hay datos en caché los seguimos mostrando
          if (this.impactos.length === 0) {
            this.sinDatos = true;
          }
      ...
      // Sin idEmpresa → no podemos cargar del backend
      this.cargando = false;
      if (this.impactos.length === 0) {
        this.sinDatos = true;
      }
  ...
  trend(actual: number, anterior: number, lowerIsBetter: boolean) {
    ...
    if (!anterior || anterior === 0) {
      return { texto: 'Sin comparativa', clase: 'trend-neutro' };
    }
    ...
    if (pct === 0) {
      return { texto: 'Sin cambio vs anterior', clase: 'trend-neutro' };
    }
  ...
  calcularIndice() {
    ...
    const pts = badges.reduce((s, b) =>
      s + (b === 'badge-verde' ? 100 : b === 'badge-amarillo' ? 50 : 10), 0);
